...
box.season.1 <- 
  ...
  xlab("Collection Season") + ylab("DBC (μg/L)") +
  scale_color_manual(values=c("#E69F00", "#56B4E9")) +
  labs(color = "Early vs. Late Season", title = "a.) 2022 DBC samples") + 
...
box.season.3 <- 
  ...
  xlab("Collection Season") + ylab("DBC (μg/L)") +
  scale_color_manual(values=c("#E69F00", "#56B4E9")) +
  labs(color = "Early vs. Late Season", title =  "b.) 2023 DBC samples") + 
  stat_summary(fun = "mean", geom = "point", shape = 4, size = 3, 
               color = "black") +
  theme(plot.title = element_text(size = 12))
box.season.3
...
p.DBC.scatter.2022.1 <- ggplot(sub.2022, aes(x=sub.2022$Sample.Collection.Date, 
  ...
  geom_point(na.rm=TRUE, shape = 19) + 
  xlab("Collection Date") + ylab("DBC (μg/L)") +
  labs(color = "Sample Type", title = "a.) 2022 DBC samples") +
...
p.DBC.scatter.2022.2 <- 
  ggplot(sub.2022, aes(x=sub.2022$Sample.Collection.Date, 
...
p.DBC.scatter.2023.1 <- 
  ggplot(sub.2023, 
         aes(x=sub.2023$Sample.Collection.Date, 
             y=sub.2023$Dittmar..DBC..µg.C.L, 
             color = sub.2023$Sample.Type)) +
  geom_point(na.rm=TRUE, shape = 19) + 
  xlab("Collection Date") + ylab("DBC (μg/L)") +
  labs(color = "Sample Type", title = "b.) 2023 DBC samples") +
  theme_minimal() + theme(panel.background = element_rect(fill = "white"), 
                          plot.background = element_rect(fill = "white")) +
  theme(plot.title = element_text(size=10)) +
...
order.2022 <- c("16-May-2022", "Jun-2022", "Jul-2022", "15-Jul-2022", 
                ...
                "15-Sept-2022 (PR)", "15-Sept-2022 (SH)", "25-Sept-2022", 
                "14-Oct-2022")
...
# boxplot BPCAs
bpca.comp.2022 <- ggplot(data.long.2022, 
                         aes(x = BPCA, y = Percentage, color = BPCA)) +
  geom_boxplot() + 
  scale_color_manual(values = 
                       c("B3CA" = "#CC79A7", "B4CA" = "#009E73", 
                         "B5CA" = "#56B4E9", "B6CA" = "#E69F00")) +
  theme_minimal() + stat_summary(fun = "mean", geom = "point", shape = 4, 
                                 size = 2, color = "black") +
...
bpca.comp.2023 <- ggplot(data.long.2023, 
                         aes(x = BPCA, y = Percentage, color = BPCA)) +
...
pca.data <- 
  sub.all.snow[, c("Sample.ID", "Sample.Location", "Sample.Type", "Year",
                   "Early.vs..Late.Season..March.July.vs..August.October.",
                   "B6CA", "B5CA", "B4CA", "B3CA")]
...
p.scores.location <- 
  ...
  geom_point(size = 3) +
  labs(x = "PC1 (96.88%)", y = "PC2 (2.90%)") +
  theme_minimal() + 
  scale_color_manual(values = 
                       c("Artist Point" = "#CC6677", 
                         "Bagley Basin" = "#332288", 
                         "Camp Kiser" = "#DDCC77", 
  ...
  geom_segment(data = loadings.df, aes(x = 0, y = 0, xend = PC1, yend = PC2), 
               ...
               inherit.aes = FALSE) +
  geom_text(data = loadings.df, aes(x = PC1, y = PC2, label = BPCA, 
...
p.SNICAR.2022 <- ggplot(SNICARmelt.2022, aes(Wavelength,value, 
  ...
  scale_color_manual(labels = c("16-May-2022", "Jun-2022", 
...
p.SNICAR.2023 <- ggplot(SNICARmelt.2023, 
  ...
  scale_color_manual(labels = c("2-Mar-2023", "6-May-2023", 
                                "20-May-2023 (AP)", 
                                "20-May-2023 (BB)", "8-Jun-2023", 
                                "26-Jun-2023", 
                                "10-Jul-2023", "17-Jul-2023", "24-Jul-2023", 
                                "6-Aug-2023", "23-Aug-2023"),
                     values = c("#332288", "#DDCC77", 
...
## double-check naming of columns in .csv files. It was renamed to be more readable and is different from the following code.
